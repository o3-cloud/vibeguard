# Sample Setup Prompt: Go Project with golangci-lint

This is an example of the setup guide generated for a Go project with golangci-lint configured.
...
**Project Name:** github.com/example/myservice
**Project Type:** go
...
- Test directories: internal (co-located)
- Entry points: cmd/myservice/main.go
...
### imports (format)
**Description:** Check import grouping and ordering with goimports
**Rationale:** Consistent import ordering improves readability
**Command:** `test -z "$(goimports -l .)"`
**Severity:** error
**Suggestion on failure:** Run 'goimports -w .' to fix import ordering.

### vet (lint)
**Description:** Run go vet to detect suspicious constructs
**Rationale:** go vet finds bugs that the compiler doesn't catch
**Command:** `go vet ./...`
**Severity:** error
...
**Rationale:** golangci-lint aggregates multiple Go linters for comprehensive analysis
**Command:** `golangci-lint run ./...`
**Severity:** error
**Suggestion on failure:** Fix linting issues. Run 'golangci-lint run --fix' for auto-fixes.
**Requires:** fmt, vet
